...
parser.add_argument(
    "--git_sha", type=str, required=True, help="git sha of the commit"
)
parser.add_argument(
    "--job_run_id",
    type=str,
    required=True,
    help="run id of the run of the databricks job",
)
parser.add_argument(
    "--branch", type=str, required=True, help="branch of the project"
)
...
logger.info(
    "Marvel model evaluation completed, model improved: %s", model_improved
)
...
if model_improved:
    ...
    code_paths = [
        f"{root_path}/artifacts/.internal/marvel_characters-{marvel_characters_v}-py3-none-any.whl"
    ]
    ...
    latest_version = wrapper.log_register_model(
        ...
        experiment_name=config.experiment_name_custom,  # type: ignore
        input_example=basic_model.X_test[0:1],  # type: ignore
        tags=tags,
