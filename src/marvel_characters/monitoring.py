...
def create_or_refresh_monitoring(
    ...
    inf_table = spark.sql(
        f"SELECT * FROM {config.catalog_name}.{config.schema_name}.`custom_model_payload`"
    )
    ...
    if inf_count == 0:
        logger.warning(
            "No records found in custom_model_payload table. Monitoring table will be empty."
        )
        return
    ...
    df_final = df_exploded.withColumn(
        ...
        F.col("request_time").alias(
            "timestamp"
        ),  # Use request_time as the timestamp
    ...
    df_final_with_status = df_final.withColumn(
        "prediction", F.col("prediction").cast("int")
    )
    ...
    if valid_count > 0:
        ...
        logger.warning(
            "All records have null prediction values. Using records with potential nulls."
        )

    # Ensure Height and Weight are properly cast to double
    df_final_with_status = df_final_with_status.withColumn(
        "Height", F.col("Height").cast("double")
    )
    df_final_with_status = df_final_with_status.withColumn(
        "Weight", F.col("Weight").cast("double")
    )

    # Log the final count before writing
    final_count = df_final_with_status.count()
    logger.info(
        f"Final record count before writing to monitoring table: {final_count}"
    )
...
def create_monitoring_table(
    ...
    spark.sql(
        f"ALTER TABLE {monitoring_table} SET TBLPROPERTIES (delta.enableChangeDataFeed = true);"
    )
